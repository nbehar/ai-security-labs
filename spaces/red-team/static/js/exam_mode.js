...
function _updateTimerDisplay() {
  ...
  if (s < 300) el.style.color = "#dc2626";
}
  ...
  container.innerHTML = `
    <div style="max-width:720px;margin:0 auto">
      ...
      <p style="font-size:0.8rem;color:var(--color-muted)">
        Question ${idx + 1} of ${total} - Bloom Level ${escapeHtml(String(q.bloom_level || ""))} - ${escapeHtml(String(q.points || 5))} pts
      </p>
    ...
    <div style="margin-bottom:32px">
      <div style="background:var(--color-surface,#1e1e22);border:1px solid var(--color-border,#3a3a3e);
        ...
        <p style="font-size:0.8rem;color:var(--color-muted);margin:0 0 8px">
          Short Answer - ${escapeHtml(String(q.points || 20))} pts (instructor-graded)
        </p>
  ...
  container.innerHTML = `
    <div style="max-width:720px;margin:0 auto">
      ...
      <p style="color:var(--color-muted);font-size:0.85rem;margin:0 0 24px">
        Graded by your instructor. Aim for 100-200 words each.
      </p>
      ${fields}
...
async function _submitTheory(container, onComplete) {
  container.innerHTML = `<p style="color:var(--color-muted)">Submitting...</p>`;
  try {
...
function _renderTheoryComplete(container) {
  container.innerHTML = `
    <div style="max-width:480px;margin:40px auto;text-align:center">
      <p style="font-size:2rem;margin:0 0 12px">[submitted]</p>
      <h3>Theory Assessment Submitted</h3>
      <p style="color:var(--color-muted)">
        MCQ answers machine-scored. Short answers will be reviewed by your instructor.
...
export async function renderReceiptTab(container) {
  container.innerHTML = `<p style="color:var(--color-muted)">Loading receipt...</p>`;
  try {
...
function _renderReceiptUI(container, receipt) {
  ...
  container.innerHTML = `
    <div style="max-width:680px;margin:0 auto">
      ...
      <p style="color:var(--color-muted);font-size:0.85rem;margin:0 0 24px">
        ${escapeHtml(receipt.exam_id || "")} - ${escapeHtml(receipt.student_id || "")} - Time: ${elapsedStr}
      </p>
