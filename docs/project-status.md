...
*Last updated: 2026-04-28 (Red Team spec fix + Educational Layer specs + L5 Guardrail + Multimodal Phase 1+2 + ZeroGPU→Inference-API pivot + Multimodal Phase 3 defenses + Phase 4a full API surface)*
...
|----------|-------|--------|------------|----------|
| 3 | Multimodal Security | **Phase 4a DEPLOYED** at `nikobehar/ai-sec-lab4-multimodal` — full API surface (8 endpoints), 4 toggleable defenses, image upload mode, scoring + leaderboard, Postman. Phase 4b (frontend SPA shell) is next. | P1 Image Prompt Injection + P5 OCR Poisoning | `cpu-basic` + HF Inference Providers (`Qwen/Qwen2.5-VL-72B-Instruct` via `ovhcloud`) |
